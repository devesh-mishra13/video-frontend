...
export default function Home() {
        ...
        formData,
        {
          ...
          onUploadProgress: (progressEvent: any) => {
            if (progressEvent.total) {
              ...
              setUploadProgress(percent); // Update the progress
            }
          },
        }
      );
  ...
  return (
    <div className="min-h-screen bg-[#0d1117] text-white p-8 font-sans relative">
      {/* Go to Home Button in the Upper Right Corner */}
      <button
        onClick={() => window.location.href = '/'}
        className=" mx-auto w-fit absolute top-8 right-8 px-6 py-3 bg-gradient-to-r from-blue-600 via-purple-700 to-indigo-600 rounded-full text-white font-semibold shadow-lg hover:shadow-2xl transition-all duration-300 text-white text-sm font-medium"
      >
        Go to Home
      </button>
      ...
      {!videosUploaded && (
        <div className="mb-8 max-w-3xl mx-auto">
          ...
          <button
            ...
            className={`px-8 py-3 rounded text-sm font-medium w-full ${
              loading ? 'bg-gray-700 cursor-not-allowed' : 'bg-[#D4AF37] hover:bg-[#C49A28]'
            } transition-all`}
          >
            {loading ? (
